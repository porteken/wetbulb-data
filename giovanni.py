...
GIOVANNI_AUTH_URL = "https://urs.earthdata.nasa.gov/api/users/find_or_create_token"
...
def _sleep_with_backoff(attempt: int, *, retry_after: str | None = None) -> None:
    ...
    time.sleep(delay + _RNG.uniform(0.0, 2.0))
...
def contiguous_year_ranges(years: list[int]) -> list[tuple[int, int]]:
...
def _fetch_cities_batch(
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        futures = {
            executor.submit(
                fetch_city_series, session, token_manager, row, fetch_ranges
            ): row.location_id
            for row in rows
        }
        for future in tqdm(
...
def process_giovanni(
    ...
    *,
    ...
    fetch_ranges = contiguous_year_ranges(pending_year_list)
    LOGGER.info(
